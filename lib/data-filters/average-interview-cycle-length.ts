...
export function getAverageInterviewCycleLength(applications: Application[]) {
  ...
  const validApplications = applicationsWithInterviews.filter((application) => {
  ...
  const averageInterviewCycleLength = totalDays / validApplications.length;

  return averageInterviewCycleLength.toFixed(0);
}
